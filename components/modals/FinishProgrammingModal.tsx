...
interface FinishProgrammingModalProps {
    show: boolean;
    onClose: () => void;
...
export const FinishProgrammingModal: React.FC<FinishProgrammingModalProps> = ({ show, onClose, onConfirm, job }) => {
    ...
    const handleInternalConfirm = () => {
        ...
        if (validDrawings.length === 0) {
            alert("Please add at least one valid program/drawing reference.");
            return;
        }
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-[60]">
            <div className="bg-white p-8 rounded-xl shadow-2xl max-w-2xl mx-auto w-full">
                <h3 className="text-2xl font-bold text-green-700 mb-4 border-b pb-3">Finish Programming for {job.jobNumber}</h3>
                <p className="text-sm text-gray-600 mb-4">Add the program/drawing numbers that have been created for this job.</p>
                    ...
                    {drawings.map((d, i) => (
                        <div key={d.id} className="bg-gray-50 p-3 rounded-lg flex items-center gap-4">
                            ...
                            <div className="flex-grow">
                                <label className="block text-xs font-medium text-gray-600">Program/Drawing Name</label>
                ...
                <button onClick={handleAddDrawing} className="mt-4 w-full p-2 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 flex items-center justify-center gap-1"><PlusIcon /> Add Another Program</button>
                
                <div className="flex justify-end space-x-3 mt-8">
                    <button onClick={onClose} className="px-5 py-2 border rounded-md">Cancel</button>
                    <button onClick={handleInternalConfirm} className="px-5 py-2 bg-green-600 text-white rounded-md">Complete Programming</button>
                </div>
            </div>
        </div>
    );
