...
import type { OfficialStaff } from '../../types';
...
interface VerifyStaffLockModalProps {
    show: boolean;
    onClose: () => void;
    staff: OfficialStaff | null;
}

export const VerifyStaffLockModal: React.FC<VerifyStaffLockModalProps> = ({ show, onClose, staff }) => {
    // PIN State
    ...
    // Effect to reset state when modal opens/staff changes
    useEffect(() => {
        if (show && staff) {
            setPinInput('');
            setPinStatus('idle');
    ...
    useEffect(() => {
        ...
        const startCamera = async () => {
            if (staff?.faceLockCode && navigator.mediaDevices?.getUserMedia) {
                try {
    ...
    const handleVerifyPin = () => {
        setPinStatus(pinInput === staff?.pin ? 'success' : 'fail');
        setTimeout(() => {
            setPinStatus('idle');
            setPinInput('');
        }, 2000);
    ...
    const handleInteractionEnd = () => {
        if (isDrawing) {
            setIsDrawing(false);
            if (path.length > 0) {
                const isMatch = staff?.patternLockCode && path.length === staff.patternLockCode.length && path.every((v, i) => v === staff.patternLockCode![i]);
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-[80]">
            <div className="bg-white p-6 rounded-xl shadow-2xl max-w-lg mx-auto w-full">
                <div className="flex justify-between items-center mb-4 border-b pb-3">
                    <h3 className="text-xl font-bold text-indigo-700">Test Security for {staff.name}</h3>
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                    ...
                    {/* Pattern Verifier */}
                    {staff.patternLockCode && staff.patternLockCode.length > 0 && (
                        <div className="bg-gray-50 p-4 rounded-lg border">
